...
%%
S = 4 * pi * R.^2 .* num;%开孔面积
DNAo = 1.3 * 10^-11;%溶液DNA浓度
Zeff = 1;%DNA有效价绝对值（以前）,现在我们认定它是一个经验参数，因为他的数值不会直接影响DNA的浓度。例如zeff=20，那么
%它的影响大概就是原来数值的10~20倍，也就是一个零。
k = 1.381 * 10^-23;%玻尔兹曼常数
e = 1.6 * 10^-19;%元电荷
Vcell = 3 * pi * 10^-18; %细胞体积，按球体算
